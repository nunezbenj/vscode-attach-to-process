...
export class ProcessItem extends vscode.TreeItem {
  constructor(public readonly proc: PythonProcess, active: boolean, injectedBefore: boolean) {
    ...
    this.iconPath = new vscode.ThemeIcon(
      active ? "debug" : proc.hidden ? "eye-closed" : proc.parsed.debugpyListen ? "broadcast" : "file-code",
    );
...
export class ProcessTree implements vscode.TreeDataProvider<ProcessItem>, vscode.Disposable {
  ...
  async getChildren(element?: ProcessItem): Promise<ProcessItem[]> {
    ...
    return procs.map((p) => new ProcessItem(p, this.deps.isActive(p.pid), this.deps.wasInjected(p.pid)));
  }
